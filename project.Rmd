...
In this case the Cohen's kappa is about 0.9958 with a standard deviation of about 0.0008.
...
We illustrate the relationship between `classe`, `num_window` and `roll_belt` using a plot with the class centres. Even though `num_window` appears as the most important variable, probably it could have been left out of the model as it seems similar to a timestamp.

```{r centre_plot, cache = TRUE}
train.center <- classCenter(data.train[,which(names(data.train) %in% c('num_window', 'roll_belt'))],
      data.train$classe, mod.rf$finalModel$prox);
train.center <- as.data.frame(train.center);
train.center$classe <- rownames(train.center);
center.plot <- qplot(num_window, roll_belt, col = classe, data = data.train);
center.plot + geom_point(aes(x=num_window, y = roll_belt, col = classe),
               size = 5, shape = 4, data = train.center);
```
